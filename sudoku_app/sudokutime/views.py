...
def index(request):
    ...
    for i in range(9):
        ...
        new_box_solved = new_sudoku_solved[0+off:3+off] + new_sudoku_solved[9+off:12+off] + new_sudoku_solved[18+off:21+off]
        row = int(i//3) + 1
        col = i % 3 + 1
        grid_area = f'grid-area: {row}/{col}/{row + 1}/{col + 1}'
        boxes.append({'box': new_box, 'box_solved': new_box_solved, 'grid_area': grid_area})
